...
go语言采用的是GMP模型，G就是go协程，M是机器线程，P是中间层：处理器。早起的golang是没有中间层P的，然而调度器和锁却是全局资源，这会导致，锁和资源竞争激烈，并且系统调用会阻塞正在运行的线程，开销比较大，因此在后续版本中加入了中间层P。每一个P有一个本地的Goroutine队列，对于本地来说，一般只会有一个p调度本地队列，因此，大部分时间不需要加锁，当然，当多个p去扫描全局队列时也还是要加锁。
